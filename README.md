...
> 3. React-Query(@tanstack/react-query)을 다양한 방법으로 사용해보고, 낙관적 업데이트 구현 방법을 충분히 이해하고 체득한다.
...
- [ ] (추가) Todo 생성 시 `Priority`(우선순위) 추가 (단계: urgent, normal, low)
  - [ ] `Priority` 추가에 따라 리스트를 우선순위를 필터링하여 볼 수 있습니다
  - [ ] 키워드를 지정하여 할 일을 생성하고 키워드에 따라 할 일을 검색할 수 있습니다
  - [ ] 생성 날짜에 따라 오름차순 혹은 내림차순으로 정렬할 수 있습니다
  - [ ] 위 조건을 URL query string을 통해 확인할 수 있습니다
